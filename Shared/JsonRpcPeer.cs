...
namespace McpSdk.Shared
{
    ...
    public sealed class JsonRpcPeer : ITransport
    {
        ...
        private readonly Dictionary<RequestId, TaskCompletionSource<IResponse>> _pending = new();
        ...
        public async Task Stop()
        {
            ...
            List<TaskCompletionSource<IResponse>> pending;
            lock (_gate)
            {
                pending = new List<TaskCompletionSource<IResponse>>(_pending.Values);
                _pending.Clear();
            }
        ...
        public Task SendOkResponse(RequestId requestId, Json result, CancellationToken cancellationToken = default)
            => _channel.Send(JsonRpcResponse.Result(requestId, result), cancellationToken);

        public Task SendErrorResponse(RequestId requestId, Error error, CancellationToken cancellationToken = default)
            => _channel.Send(JsonRpcResponse.Failure(requestId, error), cancellationToken);
            ...
            try
            {
                switch (message)
                {
                    case JsonRpcNotification notification:
                        NotificationReceived?.Invoke(notification.Method, notification.Parameters);
                        break;
                    case JsonRpcRequest request:
                        RequestReceived?.Invoke(request.Id, request.Method, request.Parameters);
                        break;
                    case JsonRpcResponse response:
                        TaskCompletionSource<IResponse> tcs;
                        lock (_gate)
                        {
                            if (_pending.TryGetValue(response.Id, out tcs))
                                _pending.Remove(response.Id);
                        }
                        tcs?.TrySetResult(response.ToResponse());
                        break;
                }
            }
            catch (Exception e)
