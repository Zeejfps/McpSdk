...
namespace McpSdk.Shared
{
    ...
    /// the legacy <see cref="JsonRpcTransport"/> and the channel/peer stack alike — encodes and classifies
    /// messages through this codec, so the wire shapes live in exactly one place. Encoding produces a
    /// <see cref="JsonRpcFrame"/> (the serialized text plus the kind/id the producer already knew, so a
    /// channel can route without re-parsing); decoding produces a <see cref="JsonRpcMessage"/>. The codec
    /// is stateless and framing-agnostic: it neither sends bytes nor correlates ids, so it composes with
    /// both the symmetric single-channel model and HTTP's request-coupled, per-connection routing.
    /// </summary>
    public sealed class JsonRpcCodec
    {
        ...
        public JsonRpcFrame EncodeRequest(RequestId id, string method, Json parameters) => JsonRpcFrame.Request(id, _json.Stringify(writer =>
        {
        ...
        public JsonRpcFrame EncodeNotification(string method, Json parameters) => JsonRpcFrame.Notification(_json.Stringify(writer =>
        {
            JsonRpcVersion.WriteTo(writer, "jsonrpc");
            method.WriteTo(writer, "method");
            if (parameters != null)
        ...
        public JsonRpcFrame EncodeResult(RequestId id, Json result) => JsonRpcFrame.Response(id, _json.Stringify(writer =>
        {
            JsonRpcVersion.WriteTo(writer, "jsonrpc");
            id.WriteTo(writer, "id");
        ...
        public JsonRpcFrame EncodeError(RequestId id, Error error) => JsonRpcFrame.Response(id, _json.Stringify(writer =>
        {
            JsonRpcVersion.WriteTo(writer, "jsonrpc");
            id.WriteTo(writer, "id");
